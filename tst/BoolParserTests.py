...
class PetriNetNodeTests(unittest.TestCase):
    ...
    def test_returns_the_expected_value(self):
        ...
        # Testando novos operadores
        self.assertEqual(BoolParser("di0 && di1").generate_function()(**digital_inputs),    True )
        self.assertEqual(BoolParser("di0 && di2").generate_function()(**digital_inputs),    False)
        self.assertEqual(BoolParser("di2 || di3").generate_function()(**digital_inputs),    True )
        self.assertEqual(BoolParser("di2 || di4").generate_function()(**digital_inputs),    False)
        self.assertEqual(BoolParser("~di1 || di2").generate_function()(**digital_inputs),   False)
        self.assertEqual(BoolParser("NOT di0").generate_function()(**digital_inputs),       False)
        self.assertEqual(BoolParser("di0 AND di1").generate_function()(**digital_inputs),   True )
        self.assertEqual(BoolParser("di0 OR di2").generate_function()(**digital_inputs),    True )
        self.assertEqual(BoolParser("di1 XOR di3").generate_function()(**digital_inputs),   False)
        self.assertEqual(BoolParser("di4 XOR di3").generate_function()(**digital_inputs),   True )
